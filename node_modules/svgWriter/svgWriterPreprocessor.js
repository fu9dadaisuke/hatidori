    ...
    function SVGWriterPreprocessor() {
        ...
        this.externalizeStyles = function (ctx) {
            var omIn = ctx.currentOMNode,
                ...
                weightMapReg = Object.getOwnPropertyNames(weightMap).join("|") + "|100|200|300|400|500|600|700|800|900",
                italicMapReg = Object.getOwnPropertyNames(italicMap).join("|");
            ...
            if (omIn.style.font && typeof omIn.style.font == "object") {
                ...
                if (fontType) {
                    styleBlock.addRule("font-family", fontType);
                }
                if (font.variant == "small-caps") {
                    styleBlock.addRule("font-variant", font.variant);
                }
                if (font.style) {
                    var fontStyle = font.style.toLowerCase().replace(/\s+/, ""),
                        fontWeight = fontStyle.match(weightMapReg),
                        fontItalic = fontStyle.match(italicMapReg);
                    if (fontWeight) {
                        styleBlock.addRule("font-weight", weightMap[fontWeight] || parseInt(fontWeight, 10));
                    }
                    if (fontItalic) {
                        styleBlock.addRule("font-style", italicMap[fontItalic]);
                    }
                }
            }
            ...
            if (omIn.style.textAttributes) {
                ...
                if (omIn.style.textAttributes.letterSpacing) {
                    styleBlock.addRule("letter-spacing", omIn.style.textAttributes.letterSpacing / 1000 + "em");
                }
        ...
        var recordBounds = function (ctx, omIn) {
                ...
                if (strokeAlign == "inside" && !(omIn.shape && omIn.shape.type in {rect: 1, ellipse: 1, circle: 1})) {
                    expand = width / 2;
            ...
            shiftBounds = function (ctx, omIn, nested, sibling) {
                ...
                // FIXME: AGC defines text as a shape. Change svgWriter
                // to follow the spec.
                if (omIn.type == "text") {
            ...
            finalizePreprocessing = function (ctx) {
                ...
                // FIXME: If we export a layer, svgWriter does not preserve the artboard
        ...
        this.processSVGNode = function (genID, ctx, noShifting, nested, sibling) {
            ...
            if (omIn.text && omIn.text.paragraphs) {
                ...
                omIn.text.paragraphs.forEach(function (paragraph, pInd) {
                    ...
                    // Ignoring filters, clipPath and mask
                    paragraph.style.filters = paragraph.style.filters || "";
                    paragraph.style.clipPath = paragraph.style.clipPath || "";
                    paragraph.style.mask = paragraph.style.mask || "";
                    ...
                    if (paragraph.lines) {
                        paragraph.lines.forEach(function (line) {
                            line.forEach(function (glyphrun, lInd) {
                                glyphrun.style = glyphrun.style || {};
                                glyphrun.style.filters = glyphrun.style.filters || "";
                                glyphrun.style.clipPath = glyphrun.style.clipPath || "";
                                glyphrun.style.mask = glyphrun.style.mask || "";
