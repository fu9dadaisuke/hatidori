...
(function () {
    ...
    var Tag = require("./svgWriterTag.js"),
        ...
        processFunctions = [
            ...
            function globalFilter(tag, ctx, parents) {
                if (!tag.globalFilter) {
                    return;
                }
                // FIXME: This code does not yet include translations caused by `trimToArtBounds`.
                var mom = parents[parents.length - 1],
                ...
                parents.forEach(function (ele) {
                    if (ele.transform) {
                        transform.preMultiplyBy(Matrix.createMatrix(ele.transform));
                    }
                });
                ...
                g = new Tag("g", {transform: getTransform(inverseTransform)});
                g.setStyleBlock(ctx, {});
                g.styleBlock.addRule("filter", filter);
                g.appendChild(tag);
                transform.preMultiplyBy(Matrix.createMatrix(tag.transform));
                tag.styleBlock.removeRule("filter");
                tag.setAttribute("transform", getTransform(transform));
                for (var i = 0; i < mom.children.length; i++) {
