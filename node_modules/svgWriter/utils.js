    ...
    function Utils() {
        ...
        function asArc(p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y) {
            var m = findDotAtBezierSegment(p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, 5),
                ...
                dim = Math.max(Math.abs(p1x - p2x), Math.abs(p1y - p2y)),
                sigma = Math.min(.05, dim / 50);
            if (arc && arc.r) {
                for (var i = 1; i < 10; i++) {
                    if (i != 5) {
        ...
        function cleanNumbers(str) {
            return str.replace(cleanNumbers.fraqAfterFraq, "$1").replace(cleanNumbers.leadZero, "$1");
        }
        cleanNumbers.commaDot = /(\.\d+),(?=\.)/g;
        cleanNumbers.leadZero = /(^|\D)0(?=\.)/g;
        cleanNumbers.fraqAfterFraq = /(\.\d+),0(?=\.)/g;
        ...
        self.optimisePath = function (path, precision, preparedPath) {
            ...
            for (i = 0; i < segs.length; i++) {
                var seg = segs[i],
                    ...
                    rel = seg.rel,
                    next = segs[i + 1],
                    loop = next && next.cmd == "z" && seg.ex == next.ex && seg.ey == next.ey;
                ...
                if (abs) {
                    ...
                    if (prev != command && !(prev == "m" && command == "l" || prev == "M" && command == "L")) {
                        res += command;
                    } else {
                        res += arg.charAt() == "-" ? "" : ",";
                    }
            ...
            return res.replace(cleanNumbers.commaDot, "$1");
        };
